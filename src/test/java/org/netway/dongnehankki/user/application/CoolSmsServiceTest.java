...
class CoolSmsServiceTest {
    ...
    void 테스트_준비() {
     ...
     void sendSms_인증번호_생성_저장_메시지_전송_확인() {
         ...
         coolSmsService.sendSms(toPhoneNumber);
    ...
    @Test
    void 본인인증시_인증코드_생성규칙_확인() {
        // given
        String toPhoneNumber = "01011112222";
        when(defaultMessageService.sendOne(any())).thenReturn(mock(SingleMessageSentResponse.class));
        ...
        String messageText = requestCaptor.getValue().getMessage().getText();
        String authCode = messageText.substring(messageText.indexOf("[", messageText.indexOf("[") + 1) + 1, messageText.lastIndexOf("]"));

        // 생성된 인증번호가 6자리 숫자인지 검증
